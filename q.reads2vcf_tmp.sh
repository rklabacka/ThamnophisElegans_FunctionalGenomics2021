...
function hard-VariantFiltration {
  ...
  #+ +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  #+ TMP COMMENT (REMOVE) # Step 4: Get rid of low-quality (mean) genotyping:
  #+ TMP COMMENT (REMOVE)   bcftools view  -i  'MIN(FMT/GQ>20)'   filtered_"$1".vcf > "$2"_HardFilterStep1.vcf
  #+ TMP COMMENT (REMOVE)   echo "Genotype Quality filtration $2 variants: $(grep -v "^#" "$2"_HardFilterStep1.vcf | wc -l)" >> Log.txt
  #+ TMP COMMENT (REMOVE) # Step 2: Get rid of multiallelic SNPs (more than 2 alleles):
  #+ TMP COMMENT (REMOVE)   bcftools view -m2 -M2 -v snps "$2"_HardFilterStep1.vcf > "$2"_HardFilterStep2.vcf
  #+ TMP COMMENT (REMOVE)   echo "Multiallelic filtration $2 variants: $(grep -v "^#" "$2"_HardFilterStep2.vcf | wc -l)" >> Log.txt
  ...
  # Step 4: Get rid of low-frequency alleles- here just singletons:
    ...
    echo "Singleton filtration $2 variants: $(grep -v "^#" "$2"_HardFilterStep4.vcf | wc -l)" >> Log.txt
  # Step 5: Get rid of low-depth individuals per site
    bcftools view  -i  'MIN(FMT/DP>9)' "$2"_HardFilterStep4.vcf > "$2"_HardFilterStep5.vcf 
  # Step 6: Get rid of low-quality (mean) genotyping (again):
    bcftools view  -i  'MIN(FMT/GQ>20)'   "$2"_HardFilterStep5.vcf > "$2"_HardFilterStep6.vcf
    echo "Genotype Quality filtration $2 variants: $(grep -v "^#" "$2"_HardFilterStep6.vcf | wc -l)" >> Log.txt
...
annotateVariants Full_Genes SeqCap
annotateVariants Full_Exons SeqCap
annotateVariants Full_CDS SeqCap
annotateVariants Full_CDS_missense SeqCap
annotateVariants Full_CDS_synonymous SeqCap
